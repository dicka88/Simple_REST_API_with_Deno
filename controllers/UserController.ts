...
class UserController {
  ...
  static updateUser = async ({
    ...
    let users: User = DataUser.filter((data) => data.fullname === params.fullname)[0];

    if (users) {
      const body = await request.body();
      const updateUser: { email?: String; fullname?: String; address?: String } = body.value;
      users = { ...users, ...updateUser };
      DataUser = [...DataUser.filter((data) => data.fullname === params.fullname), users];
